...
class MAIN(PreTrainedModel):
    ...
    def __init__(
        ...
        visual_abstractor: PreTrainedModel,
        multi_task: Optional[PreTrainedModel],
        ...
        **kwargs):
        ...
        self.visual_abstractor: MAINVisualAbstractorModel = visual_abstractor
    ...
    def _prepare_inputs_with_pixel(
        ...
        gt = input_ids.masked_fill(attention_mask == self.tokenizer.pad_token_id, -100)
        actual_input_ids = input_ids[:,:-1]

        _, T, _ = visual_hidden_states.size()
        _, K = actual_input_ids.size()
        total_number = T + K
