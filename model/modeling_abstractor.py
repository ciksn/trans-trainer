...
class MAINVisualAbstractorMLP(nn.Module):
    def __init__(self, config: MAINAbstractorConfig):
        super().__init__()
        self.config = config
...
class MAINAbstractorMultiHeadAttention(nn.Module):
    def __init__(self, config: MAINAbstractorConfig):
        super().__init__()
        self.config = config
...
class MAINVisualAbstractorCrossOutput(nn.Module):
    def __init__(self, config: MAINAbstractorConfig):
        super().__init__()
        dim = config.hidden_size
        self.out_proj = nn.Linear(dim, dim, bias=True)
        self.norm2 = nn.LayerNorm(dim)
        self.mlp = MAINVisualAbstractorMLP(config)
...
class MAINVisualAbstractorAttention(nn.Module):
    def __init__(self, config: MAINAbstractorConfig):
        super().__init__()
        self.attention = MAINAbstractorMultiHeadAttention(config)
        self.output = MAINVisualAbstractorCrossOutput(config)
...
class MAINVisualAbstractorLayer(nn.Module):
    def __init__(self, config, layer_idx):
        ...
        self.crossattention = MAINVisualAbstractorAttention(config)
...
class MAINVisualAbstractorEncoder(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.layers = nn.ModuleList(
            [MAINVisualAbstractorLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
...
class MAINVisualAbstractorModel(PreTrainedModel):
    def __init__(self, config: MAINAbstractorConfig, language_hidden_size):
        super().__init__(config)
        self.config = config

        self.encoder = MAINVisualAbstractorEncoder(config)
