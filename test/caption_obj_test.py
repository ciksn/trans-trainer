...
from eval.compute_metric import text_only_language_eval, get_iou
...
def main(config):
    ...
    for batched in tqdm(test_dataloader):
        ...
        gt_bbox = labels['bbox'] if gt_bbox is None else torch.cat([gt_bbox,labels['bbox']],dim=0)
        ...
        pred_list += tokenizer.batch_decode(pred_outputs[0],skip_special_tokens=True)
